...
import { AnimationQueueEntry } from "../../../src/types";
...
describe("createTodaysAnimations (integration, no mocks for solar/storage/date)", () => {
  ...
  it("creates all animation types for a valid webcam and date using real solar/storage/date logic", async () => {
    ...
    // Verify animation timing matches actual implementation (not comment requirements)
    ...
    // Sunrise animation should go from firstLight to sunrise (actual implementation)
    expect(sunrise).toBeDefined();
    expect(sunrise!.start_time).toBeLessThan(sunrise!.end_time);

    // Sunset animation should go from sunset to lastLight (actual implementation)
    expect(sunset).toBeDefined();
    expect(sunset!.start_time).toBeLessThan(sunset!.end_time);

    // Full day should go from firstLight to lastLight
    expect(fullDay).toBeDefined();
    expect(fullDay!.start_time).toBeLessThan(fullDay!.end_time);
  });
